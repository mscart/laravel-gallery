...
@section('content')
    <form action="{{ route('galleries.update',$gData->id) }}" method="post" enctype="multipart/form-data" class="multi_validate">
        {{ csrf_field() }}
        @method('PUT')
        <div class="card">
                ...
                <div class="row">
                            ...
                            @foreach($langs as $lang=>$name)
                                <div class="tab-pane fade show @if ($loop->first) active @endif" id="tab_{{$lang}}">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="name[{{$lang}}]" id="name_{{$lang}}" class="form-control slugify" data-remote = "slug_{{$lang}}" value="{{$gDetails[$lang]['name']}}" required=""/>
                                    </div>
                                    <div class="form-group">
                                        <label for="">@lang('galleries::gallery.gall_slug')</label>
                                        <input type="text" name="slug[{{$lang}}]" id="slug_{{$lang}}" class="form-control slug_{{$lang}}" required="" value="{{$gDetails[$lang]['slug']}}"/>
                                    </div>
                                    <div class="form-group">
                                        <label for="">@lang('galleries::gallery.gall_desc')</label>
                                        <textarea type="text" name="desc[{{$lang}}]" id="desc__{{$lang}}" class="form-control" required="">{{$gDetails[$lang]['desc']}}</textarea>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="parent_id" id="parent_id" class=" form-control select_2">
                                <option value="0">@lang('galleries::gallery.root')</option>
                                @foreach($galeries as $gal)
                                    <option @if($gal->id == $gData->id) disabled @endif @if(Route::currentRouteName()=='galleries.edit' &&  $gData->parent && $gal->id == $gData->parent->id) selected @endif value="{{$gal->id}}">{{$gal->gallery_name->first()->name}}</option>
                        ...
                        <div class="form-group">
                            ...
                            <select name="users[]" id="users" class=" form-control multiselect" multiple>

                                @foreach($users as $user)
                                    <option @if(in_array($user->id,$gUsers)) selected @endif value="{{$user->id}}">{{$user->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="active" id="active" class=" form-control select_2">
                                <option  @if($gData->active == 1) selected @endif value="1">@lang('admin/general.yes')</option>
                                <option  @if($gData->active == 0) selected @endif value="0">@lang('admin/general.no')</option>
                            </select>
                        </div>
                    </div>
                </div>
